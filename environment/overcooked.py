...
class Overcooked(Env):
    ...
    def __init__(self, layout="small", id="Overcooked-v2", max_timesteps=float("inf"), rewards=(-1, 5, 25, 100)):
...
class SingleAgentWrapper(Wrapper):
    ...
    def one_hot_encoding(self, state):
        return self.env.one_hot_encoding(state)
...
class StatesAsFramesWrapper(SingleAgentWrapper):
    def __init__(self, env: Overcooked, teammate, width=84, height=84):
        super().__init__(env, teammate)
        self.num_features = width*height
        self.width = width
        self.height = height

    def reset(self):
        super().reset()
        return self.preprocess(self.env.frame)

    def step(self, a0: int):
        next_state, reward, terminal, info = super().step(a0)
        frame = self.render(mode="silent")
        return self.preprocess(frame), reward, terminal, info

    def preprocess(self, observation):
        observation = np.dot(observation[..., :3], [0.299, 0.587, 0.114])
        observation = imresize(observation, [self.height, self.width], 'bilinear')
        observation = observation.astype(np.float32) / 128.0 - 1.0
        observation = observation[None, :, :]
        return observation

    def current_state_ohe(self):
        state = self.env.state
        return self.one_hot_encoding(state)
