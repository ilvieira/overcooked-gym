...
if __name__ == '__main__':
    ...
    save_dir = os.getcwd() + f"/data/forward_time_round{args.round}_logspace_{args.device}/" if args.data_dir is None else args.data_dir
    seed = args.seed
    if seed is None:
    ...
    datapoints_log = []
    dqn_times = []
    distilled_agent_times = []
    save_file = save_dir + "/times_comparison_in_seconds.csv"

    with open(save_file, "w") as times_file:
        times_file.write(f"NumDataPoints,DQNTime,DistillationTime")

    for n_datapoints in np.logspace(2, 5, 10, endpoint=True):
        data_st = time.time()
        # collect some states
        states = []
        env = teammates_envs[0]  # the teammate at this stage is irrelevant since the states are sampled randomly
        print(n_datapoints)
        for _ in range(int(n_datapoints)):
        ...
        # Compute the time for each of the DQNs
        dqn_st = time.time()
        for state in states:
        ...
        # Compute the time for the distilled agent
        dist_st = time.time()
        for state in states:
            distilled_agent_network.forward_for_all_tasks(state)
        dist_time = time.time() - dist_st

        with open(save_file, "a") as times_file:
            times_file.write(f"\n{n_datapoints},{dqn_time},{dist_time}")
